...
offset: 
    ...
    Move the origin to the chair (MY STANDING POSITION IN VICON of 484)

    Feel free to change if the environment changes
    OR USE TF
'''
...
class bot_stl:
    def __init__(self, name):
        ...
        self.twist_pub = rospy.Publisher('/server' + '/cmd_vel_' + str(self.name[self.name.__len__() - 1]), Twist, queue_size=1)
        self.pose_sub = rospy.Subscriber('/vrpn_client_node/' + self.name + '/pose', PoseStamped, self.pose_cb)
    ...
    def pose_cb(self, data):
        ...
        self.yaw = yaw
        self.x = data.pose.position.x + x_offset
        self.y = data.pose.position.y + y_offset
    ...
    def publish_path_4_rviz(self):
        ...
        bot_pose_t.header.frame_id = 'world'                                                                                # '/' + self.name + '/odom'
        bot_pose_t.pose.position.x = self.x - x_offset
        bot_pose_t.pose.position.y = self.y - y_offset
        bot_pose_t.pose.position.z = 0
        [bot_pose_t.pose.orientation.x, bot_pose_t.pose.orientation.y, bot_pose_t.pose.orientation.z, bot_pose_t.pose.orientation.w] = quaternion_from_euler(0., 0., self.yaw)

        self.robot_traj.poses.append(bot_pose_t)
        self.robot_traj.header.stamp = rospy.Time.now()
        self.robot_traj.header.frame_id = 'world'                                                                           # '/' + self.name + '/odom'
