    ...
    for start in nodes:
        ...
        while stack:
            ...
            try:
                ...
                #if and dfs_tree.edges[parent, child, 0]['t_max'] > list(sc):  # added
                #    continue
...
def t_aic_onestep(iwa, source, event_uo, event_o, event_c, event_uc):
    ...
    for y_node in Y_state:
        ...
        for sc in supervisior_ut:
            ...
            for u, v, data in dfs_tree.edges(data=True):     # for u, v, data in dfs_tree.out_edges(curr_node, data=True):
            ...
            i = 0
            j = 0
            #for i in range(0, sc.__len__()):
                sc_timed_t = []
                for j in range(0, t_interval.__len__()):
                    if t_interval[j] <= max_time_uo[sc[i]]:
                        sc_timed_t
            '''

            for _iter in product(sc, t_interval):
                if list(_iter)[1] <= max_time_uo[list(_iter)[0]]:
                    enforcement_curr.append(_iter)
            print(enforcement_curr)
            '''
                    # dfs遍历生成的树
                    # 起点：当前状态所有的点
                    # 目标：找到所有的可达点
                    try:
                        reachable_edge = list(dfs_aic(dfs_tree, enforcement_curr, source=curr_node))
                        print(enforcement_curr, reachable_edge)
                    except:
                        pass
            '''
...
'''
    ...
    for edge_t in dfs_tree.edges():
        ...
        try:
            event_t = dfs_tree.edges[start, end, 0]['event']
            events_in_dfstree.append(event_t)
        except:
            pass
    events_in_dfstree = set(events_in_dfstree)

    event_uo_c = (set(event_uo) & events_in_dfstree) & set(event_c)  # & 求交集
    event_uo_uc = (set(event_uo) & events_in_dfstree) & set(event_uc)
    event_o_c = (set(event_o) & events_in_dfstree) & set(event_c)
    event_o = (set(event_o) & events_in_dfstree)  # 这边有迭代操作，千万别把顺序放反了
    event_c = (set(event_c) & events_in_dfstree)
    event_uo = (set(event_uo) & events_in_dfstree)  # 但是这边要了大量的无用信息，看看能不能最后做出来然后简化


    # 建立一个不可观事件的可达树
    dfs_tree = dfs_events(iwa, event_uo, source)
    ...
    # 求出不同事件对应的时间的最大值
    max_time_uo = {}
    for event_t in list(event_uo):
        max_time_uo.update({event_t : -1})
    for edge_t in dfs_tree.edges:
        start = list(edge_t)[0]
        end   = list(edge_t)[1]
        for event_t in event_uo:
            if get_event(dfs_tree, start, end) == event_t and \
               get_t_max(dfs_tree, start, end) >  max_time_uo[event_t]:
                max_time_uo[event_t] = get_t_max(dfs_tree, start, end)


    # 接下来是对数据做一个遍历
    Y_states_set = bts_start
    for Y in Y_states_set:
        for y_node in list(Y):
            print(y_node)
'''
...
def main():
    ...
    t_aic_onestep(iwa, '0', event_uo, event_o, event_c, event_uc)
