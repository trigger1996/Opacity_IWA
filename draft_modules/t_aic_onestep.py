    ...
    for edge_t in edges:
        ...
        try:
            ...
            if event not in event_list:  # 这个是之前的版本遗留下来的，那个时候希望dfs_tree到一个可观事件然后停止，所以需要这么计算
                G0.add_edge(start, end, event=event, t_min=iwa.edges[start, end, 0]['t_min'],
                            t_max=-iwa.edges[start, end, 0][
                                't_max'])  # 这个操作很暴力，因为如果要算过可观事件的min_max距离，那只有这条可观边是可走的，那么就在计算的时候加进来，算完就删掉
                # t_min =  nx.shortest_path_length(G0, source, end, weight='t_min')
                # t_max = -nx.shortest_path_length(G0, source, end, weight='t_max')
                t_min = nx.shortest_path_length(G0, source, start, weight='t_min') + iwa.edges[start, end, 0][
                    't_min']  # 避免计算结果为0，比如从6→6
                t_max = -nx.shortest_path_length(G0, source, start, weight='t_max') + iwa.edges[start, end, 0][
                    't_max']  # 其实start→end是必经边，这个信息其实很关键
        ...
        while stack:
            ...
            try:
                ...
                for sc_t in sc:
                    event_t = list(sc_t)[0]
                    t = list(sc_t)[1]
                    if event_t == dfs_tree.edges[parent, child, 0]['event'] and dfs_tree.edges[parent, child, 0][
                        't_min'] <= t:
...
def t_aic_onestep(iwa, source, event_uo, event_o, event_c, event_uc):
    ...
    for y_node in Y_state:
            ...
            for supervisior_curr in sc_set:
                ...
                try:
                    ...
                    if is_state_listed:
                                ...
                                for sc_timed_last in sc_last:
                                    ...
                                    if event_t in event_last_list and event_t != event_last:
                                        ...
                                        if end_t > bts.edges[edge_t[0], edge_t[1], 0]['control'][index][2]:
                                            bts.edges[edge_t[0], edge_t[1], 0]['control'][index] = (
                                            event_t, start_t, end_t)
                                        # print(233)
                                        # 否则加入该控制
                                    else:
                                        event_t = list(sc_timed_t)[0]
                        ...
                        '''
                        if state_type(z_state) == state_type(last_state) and \
                           last_state[0] == z_state[0]:                                                     # set(): 构造集合, 避免排序带来的问题
                            # 更新点
                            #bts.remove_node(lastly_add_edge[0])
                            #bts.add_node(z_state)

                            # 更新边
                            #bts.remove_node(lastly_add_edge[0], lastly_add_edge[1])
                            sc_last = bts.edges[lastly_add_edge[0], lastly_add_edge[1], 0]['control']
                            ...
                            for sc_timed_t in supervisior_curr:
                                ...
                                for sc_timed_last in sc_last:
                                    ...
                                    if event_t in event_last_list and event_t != event_last:
                                        ...
                                        start_t = bts.edges[lastly_add_edge[0], lastly_add_edge[1], 0]['control'][index][1]
                                        if t_interval.index(list(sc_timed_t)[1]) == t_interval.__len__() - 1:
                                            end_t = float("inf")
                                        else:
                                            end_t = t_interval[t_interval.index(list(sc_timed_t)[1]) + 1]
                                        bts.edges[lastly_add_edge[0], lastly_add_edge[1], 0]['control'][index] = (event_t, start_t, end_t)
                                    # 否则加入该控制
                                    else:
                                        event_t = list(sc_timed_t)[0]
                                        start_t = list(sc_timed_t)[1]
                                        if t_interval.index(list(sc_timed_t)[1]) == t_interval.__len__() - 1:
                                            ...
                                            end_t = t_interval[t_interval.index(list(sc_timed_t)[1]) + 1]
                                        bts.edges[lastly_add_edge[0], lastly_add_edge[1], 0]['control'].append((event_t, start_t, end_t))
                        '''
                    else:
                        bts.add_node(z_state)
        ...
        # 这段代码没啥用
        maxIndex = 1  # Here
        for u, v, data in dfs_tree.in_edges(curr_node,
                                            data=True):  # https://stackoverflow.com/questions/37230751/networkx-get-all-in-edges-of-a-node
            print(u, v, data)
